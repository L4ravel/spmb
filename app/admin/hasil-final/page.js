...
const RecBadge = ({ rec }) => {
  ...
  return (
    <span
      className={`inline-flex items-center rounded px-2 py-0.5 text-xs font-medium ring-1 ${cls}`}
    >
      {R || "-"}
    </span>
  );
...
export default function HasilFinalPage() {
      ...
      try {
        // users_app
        {
          ...
          while (true) {
            ...
            snap.forEach(
              (d) =>
                d.data()?.registrationLevel &&
                setLv.add(d.data().registrationLevel)
            );
        ...
        // tahfidz_scores
        {
          ...
          const snap = await getDocs(
            query(colRef, orderBy(documentId()), limit(200))
          );
          snap.forEach((d) => d.data()?.level && setLv.add(d.data().level));
        }
      } finally {
  ...
  function buildUsersQuery(afterDoc = null) {
    const colRef = collection(db, USERS_COLLECTION);
    const clauses = [
      where("registrationPaymentStatus", "in", VERIFIED_VARIANTS),
    ];
    if (levelFilter !== "ALL")
...
const isPiatu = ibuIncome.length === 0;
  ...
  const handleDownloadExcel = async () => {
    ...
    try {
      ...
      if (levelFilter !== "ALL")
        baseClauses.push(where("registrationLevel", "==", levelFilter));
...
return {
  ...
  "Nama": r.name,
  "Jenjang": r.level,
  "Yatim": r.isYatim ? "Ya" : "Tidak",
  "Piatu": r.isPiatu ? "Ya" : "Tidak",
  "Akademik": r.akademik ?? "-",
  "Al Qur'an": r.tahfidz ?? "-",
  ...
  "Status": r.complete ? "Lengkap" : "Belum Lengkap",
      ...
      const suffixDir =
        rankDirection === "DESC" ? "_TopToBottom" : "_BottomToTop";
      XLSX.writeFile(
        wb,
        `Hasil_Final_Verified${suffixLv}${suffixSt}${suffixDir}_${ts}.xlsx`
      );
    } finally {
      ...
      <main className="flex-1 min-h-0 w-full max-w-none px-4 md:px-6 lg:px-8 py-8">
        ...
        <div className="flex items-center justify-between">
          <h1 className="text-xl sm:text-2xl font-bold tracking-tight">
            Hasil Tes Akademik, Al-Qur&apos;an & Wawancara
          </h1>
          <div className="text-xs text-slate-600">
            Halaman <b>{pageIndex + 1}</b> • Baris: <b>{rows.length}</b> /{" "}
            {PAGE_SIZE}
          </div>
        </div>

        {errMsg && (
          <div className="mt-3 rounded-xl border border-rose-200 bg-rose-50 px-3 py-2 text-sm text-rose-700 shadow-sm">
            {errMsg}
          </div>
        )}
        ...
        <div className="mt-4 grid gap-4 md:grid-cols-3">
          <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-lg shadow-slate-100/70 md:col-span-2">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <select
                value={rankDirection}
                onChange={(e) => setRankDirection(e.target.value)}
                className="rounded-xl border border-slate-300 px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-slate-100"
                    ...
                    >
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2">
                          ...
                          {r.isYatim && (
                            <span className="inline-flex items-center rounded-full border border-orange-200 bg-orange-100 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-orange-700">
                              Yatim
                            </span>
                          )}
                          {r.isPiatu && (
                            <span className="inline-flex items-center rounded-full border border-blue-200 bg-blue-100 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-blue-700">
                              Piatu
                            </span>
                          )}
                        </div>
                      ...
                      <td className="px-4 py-3">
                        <RecBadge rec={r.tahfidzRecommendation} />
                      </td>
                      <td className="px-4 py-3 text-right tabular-nums">
                        {r.wawancara ?? "-"}
                      </td>
                      <td className="px-4 py-3 text-right font-semibold tabular-nums">
                        {r.total?.toFixed?.(1) ?? r.total}
                      </td>
                      ...
                      <div className="mx-auto mb-2 h-12 w-12 rounded-full bg-slate-100 flex items-center justify-center">
                        ...
                        >
                          <path
                            ...
                            strokeWidth={2}
                            d="M20 13V6a2 2 0 00-2-2H6a2 2 0 00-2 2v7m16 0v5a2 2 0 01-2 2H6a2 2 0 01-2-2v-5m16 0h-2"
                          />
                        </svg>
                      </div>
              ...
              <div className="px-5 py-4 text-sm">
                ...
                <div className="mt-3 space-y-3 text-black">
                  <section className="rounded-xl border border-slate-200 p-4">
                    ...
                    <div>
                      Nilai: <b>{detail.tahfidz ?? "-"}</b>
                    </div>
                    <div>
                      Jumlah Hafalan (Juz):{" "}
                      <b>{detail.memorizedCount ?? "-"}</b>
                    </div>
                    <div className="mt-1">
                      Rekomendasi:{" "}
                      <RecBadge rec={detail.tahfidzRecommendation} />
                    </div>
                    <div className="text-slate-600 mt-1">
                      Penguji: <b>{detail.tahfidzExaminer ?? "-"}</b>
                    </div>
                  </section>
                  ...
                  <section className="rounded-xl border border-slate-200 p-4">
                    ...
                    <div>
                      Nilai: <b>{detail.wawancara ?? "-"}</b>
                    </div>
                    <div className="text-slate-600 mt-1">
                      Penanya: <b>{detail.wawancaraExaminer ?? "-"}</b>
                    </div>
                  </section>
