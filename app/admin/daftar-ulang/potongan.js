...
export function KonfirmasiPotonganPanel({
  ...
  const [choice, setChoice] = useState(""); // "BP3" | "SPP" | "BOTH"
  ...
  const detail = useMemo(() => {
    ...
    const aktifAmountBP3 =
      currentDiscount && (aktifType.includes("BP3") || currentDiscount?.amountBP3 != null)
        ? Number(
            currentDiscount.amountBP3 ??
              (aktifType === "BP3" ? aktifAmount : 0)
          )
        : 0;

    const aktifAmountSPP =
      currentDiscount && (aktifType.includes("SPP") || currentDiscount?.amountSPP != null)
        ? Number(
            currentDiscount.amountSPP ??
              (aktifType === "SPP" ? aktifAmount : 0)
          )
        : 0;

    let totalSesudahPot = totalSebelumPotongan;
    if (aktifAmountBP3 > 0) {
      totalSesudahPot -= Math.min(aktifAmountBP3, maxPotBP3);
    }
    if (aktifAmountSPP > 0) {
      totalSesudahPot -= Math.min(aktifAmountSPP, maxPotSPP);
    }
    totalSesudahPot = Math.max(totalSesudahPot, 0);
  ...
  useEffect(() => {
    ...
    const bp3 =
      fees?.uangPangkal && typeof fees.uangPangkal?.bp3 === "number"
        ...
        : 0;
    const sppNominal = typeof fees?.spp === "number" ? fees.spp : 0;

    if (choice === "BP3") {
      setAmount(bp3 || 0);
    } else if (choice === "SPP") {
      setAmount(sppNominal || 0);
    } else if (choice === "BOTH") {
      setAmount((bp3 || 0) + (sppNominal || 0));
    } else {
      setAmount(0);
    }
  ...
  const onApplyDiscount = useCallback(
    async () => {
      ...
      if (isNonPTK) {        
        ...
        if (choice !== "BP3" && choice !== "SPP" && choice !== "BOTH")
          return alert("Pilih jenis potongan (BP3, SPP, atau keduanya).");
      }
    ...
    note: "Potongan NON_PTK (BP3 atau SPP) untuk yang bersaudara",
          ...
          const bp3Nominal =
            choice === "BP3" || choice === "BOTH"
              ? fees?.uangPangkal &&
                typeof fees.uangPangkal?.bp3 === "number"
                ? fees.uangPangkal.bp3
                : 0
              : 0;
          const sppNominal =
            choice === "SPP" || choice === "BOTH"
              ? typeof fees?.spp === "number"
                ? fees.spp
                : 0
              : 0;

          const type = choice === "BOTH" ? "BP3+SPP" : choice;

          payload = {
            type,
            amount: amount || bp3Nominal + sppNominal,
            amountBP3: bp3Nominal,
            amountSPP: sppNominal,
            sourceFeeRef: fees?.id || null,
            sourceKey:
              choice === "BP3"
                ? "uangPangkal.bp3"
  ...
  async function sendWaPtkNotification(status) {
    try {
      ...
      const nama =
        selected?.name || p.namaSantri || p.nama || p.namaLengkap || "-";

      const jenjang =
        regLevel || p.registrationLevel || p.jenjang || p.tingkat || "-";
  ...
  // Toggle pilihan BP3 / SPP (PTK boleh pilih dua, NON_PTK tetap BP3 saja)
  const computeNextChoice = (prev, target) => {
    if (!prev) return target;
    if (prev === target) return "";
    if (prev === "BOTH") {
      return target === "BP3" ? "SPP" : "BP3";
    }
    if (
      (prev === "BP3" && target === "SPP") ||
      (prev === "SPP" && target === "BP3")
  ...
  const handleSelectBP3 = () => {
    if (isNonPTK) {
      setChoice("BP3");
      return;
    }
    setChoice((prev) => computeNextChoice(prev || "", "BP3"));
  };
